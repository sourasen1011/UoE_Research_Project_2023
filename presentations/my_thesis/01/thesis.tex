...
This implementation is equipped with a wrapper for the Python library \texttt{shap} which allows explanation of particular observations. Figure \ref{fig:shap} shows the breakdown for one patient over a particular discrete time period. 
...
From here, it may be natural to want to investigate the factors that deviate an individual subject's survival (at any particular timestep) from the `expected' survival at that time.
\begin{table*}
  \centering
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{example_survival_graph.pdf}
