...
This architecture generates confidence intervals by applying Monte-Carlo dropout. For any particular subject, the returned survival curve natively has confidence bands associated with it. An example is shown in Figure \ref{fig:example_survival_graph}.
...
Further, it is also equipped with a wrapper for the Python library \texttt{shap} which allows explanation of particular observations. Figure \ref{fig:shap} shows the breakdown for one patient over a particular discrete time period.
