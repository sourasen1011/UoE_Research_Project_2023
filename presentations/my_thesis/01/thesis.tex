...
\begin{figure}[H]
  ...
  \caption{Kaplan-Meier curve of Survival for entire cohort}
  \label{fig:km_curve}
\end{figure}
...
\begin{table}
  ...
  \begin{tabular}{|p{2cm}|p{3cm}|p{1.4cm}|p{1cm}|}
    ...
    \hline
  \end{tabular}
  \caption{Evaluation metrics - Degradation with extensions. Note these metrics were collected on one specific run. Reruns may yield slightly different results}
...
\begin{table*}
  ...
  \begin{tabular}{|p{4cm}|p{10cm}|p{1.5cm}|p{1.5cm}|}
    ...
    \hline
  \end{tabular}
  \caption{Evaluation metrics - Best metrics are written in \textbf{bold}. Both metrics are reported as \texttt{Mean (lower 90\% confidence interval, upper 90\% confidence interval)}.}
  \label{tab:eval}
\end{table*}
