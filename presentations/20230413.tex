...
SHAP and LIME can be used to add interpretability to the neural network to understand how the various covariates affect the prediction of HF vs non-HF.
\item What are the trade-offs between HMM and HDL with respect to interpretability and performance?\\

Generally, HMM being a natively linear model, can perhaps not capture embedded non-linearities in the data (it is important to note that non-linear extensions have been suggested \cite{bobicknonlinear}). However, that is weighed with the fact that the model is highly transparent. Neural networks on the other hand may offer better performance, but the level of explainability suffers.
\end{enumerate}
...
\caption{\label{fig:gantt}A caption}
\end{figure}
